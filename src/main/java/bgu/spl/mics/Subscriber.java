...
public abstract class Subscriber extends RunnableSubPub {
    ...
    protected final <T, E extends Event<T>> void subscribeEvent(Class<E> type, Callback<E> callback) {
        callBackMap.put(type,callback);//insert UNIQUE key to the map.(all the event of the same type have the same callback) type=type of event.
    ...
    protected final <B extends Broadcast> void subscribeBroadcast(Class<B> type, Callback<B> callback) {
        callBackMap.put(type,callback);//insert UNIQUE key to the map.(all the BROADCAST of the same type have the same callback)
    ...
    public final void run() {
        messageBroker.register(this);//allocate queue for this.
        initialize();//register this to nececerry topics it need to recieve.
        while (!terminated) {//main loop
          try{
              Message nextMessage=messageBroker.awaitMessage(this);//waiting for new message.
              if (nextMessage!=null) {
                  Callback callback = callBackMap.get(nextMessage.getClass());
                  callback.call(nextMessage);
              }
          }
          catch (InterruptedException a){}//need to see the instructions.
        }
